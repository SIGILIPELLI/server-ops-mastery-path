...
**Why centralized logging needs a shipping agent, not just a shared
filesystem.** Logs are generated by writing to a local file descriptor on
each host; getting them into one searchable place requires an agent
(Filebeat, Fluentd, Vector) that tails each log file (tracking file offset
so it survives restarts), parses/structures each line, and ships it over the
network to a central store. This decoupling — write locally first, ship
asynchronously — exists so that logging never blocks the application on
network I/O to a remote logging service; if the shipper or network is down,
logs simply queue locally (or on disk) until it recovers, rather than
stalling the application that's producing them.

**Structured (JSON) logs vs plain text — the actual query cost
difference.** A centralized log store ingests each line and, for JSON logs,
can index individual fields (`level`, `request_id`, `status_code`)
independently, so a query like `status_code >= 500` is a fast indexed
lookup. Free-text logs require either regex parsing at ingest time (fragile,
breaks silently when a log format changes) or full-text search across the
raw string at query time, which is dramatically more expensive at scale —
the structural difference is exactly what separates a
grep-through-flat-files workflow from a real log-analytics query.

**Correlation IDs and distributed request tracing.** A single user request
touching a proxy, an app server, and a database each produces separate log
lines on separate hosts with no inherent link between them — a correlation
ID (a UUID generated at the edge and propagated through headers/context on
every downstream call) is the only thing that lets you reconstruct "every
